...
watchEffect(() => {
    if (props.show) {
        ...
        //     form[element] =  props.Procesoa[element]
        // });
        form.errors = {}
        props.titulos.forEach(names => {
            form[names['order']] = props.Procesoa[names['order']]
        });

        // form.codigo = props.Procesoa?.codigo
    }
})

const update = () => {
    form.put(route('Proceso.update', props.Procesoa?.id), {
        preserveScroll: true,
        onSuccess: () => {
            emit("close")
            form.reset()
